...
from tuesday.core import grid_baselines, thermal_noise, sample_lc_noise

def test_lc_noise_sampling():
    """Test the grid_baselines function."""

    observatory = Observatory.from_ska("AA4")
    hours_tracking = 1. * un.hour
    integration_time = 120. * un.second
    ...
    # Call the function
    proj_bls = observatory.projected_baselines(baselines=baselines,
    ...
    sigmaN_rms = thermal_noise(obs, freqs, boxlength, lc_shape, A_eff = [517.7]*un.m**2)
    sigmaN_rms = thermal_noise(obs, freqs, boxlength, lc_shape)
    sigmaN = sigmaN_rms / np.sqrt(uv_coverage * obs.n_days)
    sigmaN[uv_coverage == 0.] = 0.

    sample_lc_noise(sigmaN, 
                        seed=4, 
                        nsamples=10)
