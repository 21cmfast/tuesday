...
from tuesday.core import calculate_ps_lc, plot_power_spectrum, SphericalPS, CylindricalPS
...
def test_1d_ps_plot(ps):
    ...
    with np.testing.assert_raises(ValueError):
        bad_ps = SphericalPS(np.append(ps.ps[None,...], ps.ps[None,...], axis = 0),
                             k = ps.k)
        plot_power_spectrum(bad_ps) # Passing 2 1D PS

def test_bad_1d_ps_units(ps):
    with np.testing.assert_raises(ValueError):
        bad_ps = SphericalPS(ps.ps * un.mK**2 * un.Mpc**2,
                             k = ps.k)
        plot_power_spectrum(bad_ps)  # Wrong units on PS
    with np.testing.assert_raises(ValueError):
        bad_ps = SphericalPS(ps.ps,
                             k = ps.k / un.Mpc**4)
...
def test_good_1d_ps_units(ps, unit):
    good_ps = SphericalPS(ps.ps.value*un.mK**2*unit**3,
                        k = ps.k,
                        delta=False)
    plot_power_spectrum(good_ps)
    good_ps = SphericalPS(ps.ps.value*unit**3,
                        k = ps.k,
                        delta=False)
    plot_power_spectrum(good_ps)
...
def test_2d_ps_units(ps2):
    with np.testing.assert_raises(ValueError):
        bad_ps = CylindricalPS(ps2.ps.value * un.Mpc,
                             kperp = ps2.kperp,
                             kpar = ps2.kpar)
        plot_power_spectrum(bad_ps)  # Wrong units on PS
    with np.testing.assert_raises(ValueError):
        bad_ps = CylindricalPS(ps2.ps,
                             kperp = ps2.kperp * un.mK,
                             kpar = ps2.kpar)
        plot_power_spectrum(bad_ps)  # Wrong units on k
    with np.testing.assert_raises(ValueError):
        bad_ps = CylindricalPS(ps2.ps,
